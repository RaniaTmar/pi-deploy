...
@Component({
  ...
  styles: [`
    ...
    .stat-icon.past { background: #dcfce7; color: var(--success); }
    ...
    .campaign-card:hover .card-image { transform: scale(1.06); }
    ...
    .card-badge.active { background: var(--success); color: var(--white); }
    .card-badge.inactive { background: rgba(0,0,0,.42); color: var(--white); }
    ...
    .donor-count svg { width: 14px; height: 14px; color: var(--primary); }
    ...
    .donate-btn svg { width: 14px; height: 14px; }
    ...
    @media (max-width: 640px) {
      .donation-page { padding: 24px 18px 56px; }
...
export class DonationListComponent implements OnInit {
  ...
  get filteredCampaigns(): DonationCampaign[] {
  ...
  get totalPages(): number { return Math.ceil(this.filteredCampaigns.length / this.pageSize); }
  get pageNumbers(): number[] { return Array.from({ length: this.totalPages }, (_, i) => i + 1); }
  get pagedCampaigns(): DonationCampaign[] {
